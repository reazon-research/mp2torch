...
def overlap_similarity(
    box: torch.Tensor | SegmentedTensor, other_boxes: torch.Tensor | SegmentedTensor
) -> torch.Tensor | SegmentedTensor:
    """Computes the IOU between a bounding box and set of other boxes."""
    if box.ndim == 2:
        return box_iou(boxes1=box, boxes2=other_boxes)
    return box_iou(boxes1=box.unsqueeze(0), boxes2=other_boxes).squeeze(0)
